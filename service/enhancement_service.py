...
class EnhancementService:
    ...
    def _calculate_cost(grade_id: int, current_level: int) -> int:
        ...
        level_mult = 1.0 + (current_level * 0.3)  # 레벨당 +30%
        cost = int(EnhancementService.BASE_COST * grade_mult * level_mult)
        return cost
    ...
    async def get_enhancement_info(
        ...
        if current_level >= EnhancementService.MAX_ENHANCEMENT:
            return {
                "can_enhance": False,
                "reason": f"최대 강화 레벨입니다 (+{EnhancementService.MAX_ENHANCEMENT})"
            }
    ...
    async def attempt_enhancement(
        ...
        current_level = inv_item.enhancement_level
        if current_level >= EnhancementService.MAX_ENHANCEMENT:
            raise ValueError(f"최대 강화 레벨입니다 (+{EnhancementService.MAX_ENHANCEMENT})")
        ...
        if success:
            ...
            if current_level <= 3:
                ...
                if destruction_roll < EnhancementService.DESTRUCTION_RATE:
                    # 파괴 (20%)
