...
CUHABot is a Korean Discord bot with dungeon exploration and Baekjoon (competitive programming platform) integration features. The bot uses Discord.py for Discord API interactions and Tortoise ORM for MySQL database operations.
...
- `dungeon_command.py` - Dungeon exploration commands
- `baekjoon_command.py` - Baekjoon profile and integration commands
...
- `models/repos/static_cache.py` - In-memory cache for frequently accessed static data (dungeons, monsters, items, skills) loaded at bot startup
...
- `user_service.py` - User profile and Baekjoon integration logic
- `service/dungeon/` - Dungeon subsystem:
...
- `register_form.py` - User registration modal
- `solved_ac_profile_pagination.py` - Paginated profile view

**decorator/** - Custom decorators:
- `account.py` - `@requires_registration()` decorator to enforce user registration before command use
...
1. **Command Invocation**: User invokes slash command → Discord.py routes to appropriate Cog
2. **Authorization**: `@requires_registration()` decorator checks if user exists in database
...
5. **Response**: Service returns result → Cog sends Discord embed/view to user
...
### Database ORM
- Uses Tortoise ORM with async/await
...
- Most user-facing text is in Korean (명령어, 던전, etc.)
- Uses Korean variable names in some places (`던전`, `백준`, etc.)
- Logging configured at INFO level with timestamp format

---

# 🛠️ Development Guidelines (개발 지침)

## 필수 참고 문서

**개발 전 반드시 확인:**
- `docs/TODO.md` - 우선순위별 기능 목록 및 진행 상황
- `docs/GameOverview.md` - 게임 시스템 전체 기획
- `docs/Items.md` - 아이템/장비 기획
- `docs/Skills.md` - 스킬 시스템 기획
- `docs/Dungeons.md` - 던전 기획
- `docs/Monsters.md` - 몬스터 기획
- `docs/Balance.md` - 밸런스 수치
...
예시:
feat: 턴제 전투 루프 구현
fix: 데미지 계산 시 방어력 무시 버그 수정
test: DamageCalculator 유닛 테스트 추가
```

### PR 규칙
1. `feature/*` → `master` PR 생성
2. 모든 테스트 통과 필수
3. TODO.md 업데이트 확인
4. 최소 1명 리뷰 (가능한 경우)
...
async def use_skill(self, user_id: int, skill_id: int) -> SkillResult:
    ...
    if not user.can_use_skill(skill):
        raise InsufficientResourceError("MP 부족")

    # Main logic (중첩 없이 flat하게)
...
│   ├── test_skill_component.py
│   └── ...
├── integration/             # 통합 테스트
│   ├── test_combat_flow.py
│   ├── test_dungeon_explore.py
│   └── ...
├── e2e/                     # E2E 테스트
│   ├── test_full_dungeon_run.py
...
class TestDamageCalculator:
    ...
    def test_physical_damage_with_armor_penetration(self):
        ...
        # 방어력 50% 무시: 100 - (100 * 0.5) * 0.5 = 75
        assert damage == 75

    def test_physical_damage_max_armor_penetration(self):
        """최대 방어력 관통 (70% 제한)"""
        damage = calc.calculate_physical(
            attack=100, defense=100, armor_pen=1.0  # 100% 요청
        )
        # 최대 70%만 적용
...
class TestAttributeSystem:
    ...
    @pytest.mark.parametrize("attacker_attr,defender_attr,expected_mult", [
        (Attribute.FIRE, Attribute.ICE, 1.5),      # 강점
        (Attribute.ICE, Attribute.FIRE, 0.5),      # 약점
        (Attribute.FIRE, Attribute.FIRE, 0.5),    # 동일
        (Attribute.HOLY, Attribute.DARK, 1.5),    # 강점
        (Attribute.DARK, Attribute.HOLY, 1.5),    # 상호 강점
        (Attribute.NONE, Attribute.FIRE, 1.0),    # 무속성
    ])
...
class TestStatusEffect:
    ...
    def test_burn_stacking(self):
        ...
        for i in range(7):  # 7번 적용 시도
            BurnEffect.apply_to(target)

        assert target.get_effect_stacks(EffectType.BURN) == 5  # 최대 5
...
class TestCombatFlow:
    ...
    async def test_full_combat_victory(self, combat_session):
        ...
        while session.state == CombatState.IN_PROGRESS:
            ...
            assert result.turn_number <= 100  # 무한루프 방지
    ...
    async def test_combat_with_skill_deck(self, combat_session):
        ...
        session.user.skill_deck = [
            Skill("강타", slots=3),      # 30% 확률
            Skill("회복", slots=2),      # 20% 확률
            Skill("화염구", slots=5),    # 50% 확률
        ]
        ...
        assert 0.25 <= skill_usage["화염구"] / total <= 0.55  # ~50%
```
...
class TestUserJourneyE2E:
    ...
    async def test_new_user_first_dungeon(self, bot_client):
        ...
        # 1. 회원가입
        response = await client.send_command("/등록")
        assert "등록 완료" in response.content

        # 2. 초기 장비 확인
        response = await client.send_command("/인벤토리")
        assert "초보자의 검" in response.content

        # 3. 던전 선택
        response = await client.send_command("/던전")
        view = response.view
        assert "잊혀진 숲" in view.options

        # 4. 던전 입장
        await view.select("잊혀진 숲")
        assert "잊혀진 숲에 입장했습니다" in response.content

        # 5. 탐험
        await client.click_button("탐험")

        # 6. 전투 발생
        response = await client.wait_for_message(timeout=5)
        if "몬스터 출현" in response.content:
            await client.click_button("전투")

            # 전투 완료 대기
            response = await client.wait_for_message(
                check=lambda m: "승리" in m.content or "패배" in m.content,
                timeout=30
            )

        # 7. 보상 확인
        if "승리" in response.content:
            assert "경험치" in response.content
            assert "골드" in response.content
```
...
### 테스트
- [ ] 유닛: InventoryService 테스트
- [ ] 유닛: 아이템 정렬 테스트
- [ ] 통합: 아이템 획득→인벤토리 저장 흐름
- [ ] E2E: 인벤토리 조회 커맨드 테스트
...
CUHABot/
├── bot.py                 # 진입점
├── cogs/                  # Discord 커맨드 (Thin Layer)
│   └── {feature}_command.py
├── service/               # 비즈니스 로직 (핵심)
│   ├── {feature}_service.py
│   └── {subsystem}/       # 복잡한 시스템은 하위 디렉토리
│       ├── __init__.py
│       ├── {component}.py
│       └── ...
├── models/                # DB 모델
...
async def use_item(self, user_id: int, item_id: int) -> ItemResult:
    ...
    if session and session.in_combat:
        raise CombatRestrictionError("전투 중에는 아이템을 사용할 수 없습니다")

    # ... 로직

# Cog 레이어
@app_commands.command()
async def 아이템사용(self, interaction: Interaction, item_id: int):
    try:
        ...
        await interaction.response.send_message("❌ 먼저 등록해주세요", ephemeral=True)
```
...
async def calculate_damage(
    ...
    *,
    ...
    Args:
        attacker: 공격자 엔티티
        defender: 방어자 엔티티
        skill: 사용 스킬 (없으면 기본 공격)
        ignore_defense: 방어력 무시 비율 (0.0~0.7)
    ...
    Raises:
        InvalidStateError: 공격자가 행동 불가 상태일 때
    """
    ...
```
...
⚠️ **이 규칙들은 반드시 코드로 강제해야 함:**

1. **전투 중 아이템 사용 불가**
   ```python
   if session.in_combat:
       raise CombatRestrictionError("전투 중 아이템 사용 불가")
   ```

2. **전투 중 스킬 변경 불가**
   ```python
   if session.in_combat:
       raise CombatRestrictionError("전투 중 스킬 변경 불가")
   ```

3. **주간 타워: 아이템 일절 불가**
   ```python
   if session.content_type == ContentType.WEEKLY_TOWER:
       raise ContentRestrictionError("타워에서는 아이템 사용 불가")
   ```

4. **주간 타워: 층 사이에서만 스킬 변경**
   ```python
   if session.content_type == ContentType.WEEKLY_TOWER:
       if session.in_combat:
           raise ContentRestrictionError("층 사이에서만 스킬 변경 가능")
   ```
...
5. **패시브도 10슬롯 안에 장착**
   ...
   if total_slots > 10:
       raise DeckError("스킬 슬롯은 최대 10개입니다")
   ```
