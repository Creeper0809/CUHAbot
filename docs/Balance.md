...
> **능력치 시스템 상세는 [Stats.md](Stats.md) 참조**
> 5대 기본 능력치(STR/INT/DEX/VIT/LUK) → 전투 스탯 변환 + 시너지 효과
...
> 자연 회복은 **최대 HP 비례 %** 방식. VIT 능력치로 회복률 증가. ([Stats.md](Stats.md) 참조)

### HP 자연 회복 공식
```
회복량/분 = 최대 HP × (기본 회복률 + VIT 보너스)
...
### 회복 밸런스
```
완전 회복 예상 시간 (VIT 0, HP 300 기준):
- HP 1 → 300 (사망 후): 약 100분 (3 HP/분, 1%)
- HP 150 → 300: 약 50분

완전 회복 예상 시간 (VIT 50, HP 1000 기준):
- HP 1 → 1000 (사망 후): 약 33분 (30 HP/분, 3%)
- HP 500 → 1000: 약 17분
