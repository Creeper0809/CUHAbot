...
덱에 장착한 스킬 구성에 따라 **자동으로 시너지 효과**가 발동합니다.

### 속성 집중 시너지

**덱 내 같은 속성 스킬 개수**에 따른 보너스:

| 속성 | 3개 | 5개 | 7개 | 10개 (마스터리) |
|------|-----|-----|-----|----------------|
| 🔥 화염 | 화염 +10% | 화염 +20%, 화상+1턴 | 화염 +35%, 화상 확률 +20% | 모든 공격 화상 부여, 화상 2배 |
| ❄️ 냉기 | 냉기 +10% | 냉기 +20%, 동결+10% | 냉기 +35%, 동결 확률 2배 | 모든 공격 둔화, 50% 동결 |
...
| 시너지 이름 | 조건 | 효과 |
|-------------|------|------|
| **원소 조화** | 화/냉/전/수 각 2개+ | 모든 속성 +15% |
| **빛과 어둠** | 신성 4개+ & 암흑 4개+ | 데미지 +25%, 흡혈 +10% |
| **글래스 캐논** | 공격 스킬 8개+ | 데미지 +40%, 받는 피해 +20% |
| **철벽 방어** | 힐/버프 스킬 7개+ | 받는 피해 -25%, 데미지 -30% |
| **버서커** | 공격 5개+ & 분노/광폭화 포함 | 공격력 +15%, HP 50% 이하 시 +30% |
| **서바이버** | 힐 스킬 4개+ | 받는 피해 -15%, 힐 +20% |
| **디버프 마스터** | 디버프 스킬 4개+ | 디버프 지속 +2턴, 적용률 +20% |
...
- 같은 종류의 시너지는 중첩되지 않음
- 다른 종류의 시너지는 모두 적용됨
- 마스터리(10개 집중)는 다른 시너지와 함께 적용 불가

예시: 화염 7개 + 공격 스킬 8개
→ 화염 시너지(7개) + 글래스 캐논 = 화염 +35% & 데미지 +40%
```
...
#### 🔥 화염 버서커 덱
```
덱: 화염구×3 + 화염폭발×2 + 분노×2 + 강타×2 + 응급처치×1
...
시너지: 화염 5개 → 화염 +20%, 화상+1턴
버서커 시너지: 분노 포함 → 추가 공격력 +15%

최종 기대 DPS: ~95% (시너지 적용 시 ~115%)
생존력: 하 (힐 10% 확률)
```

#### ❄️ 빙결 컨트롤 덱
```
덱: 얼음화살×2 + 빙결×3 + 눈보라×2 + 치유×2 + 수비태세×1
...
#### 💚 불멸의 탱커 덱
```
덱: 응급처치×2 + 치유×2 + 재생×2 + 수비태세×2 + 어둠갑옷×1 + 강타×1
...
| 세트 | 덱 조합 | 결과 |
|------|---------|------|
| 드래곤 6세트 | 번개 덱 7개 | 피격 반격 + 연쇄 = 카운터 빌드 |
| 심연 8세트 | 암흑 덱 7개 | 처치 시 능력 흡수 + 영구 성장 |
| 천상 8세트 | 신성 덱 7개 | 부활 + 디버프 면역 = 불사 빌드 |
| 화염 5세트 | 화염 덱 5개 | 화상 자동 부여 + 화염 데미지 극대화 |
